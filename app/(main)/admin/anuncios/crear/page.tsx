...
const VALIDACION = {
  ...
  MENSAJE_MIN: 10,
  MENSAJE_MAX: 5000,
  MAX_DIAS_FUTURO: 365, // Máximo 1 año en el futuro
};

// Caracteres no permitidos (potencialmente peligrosos)
...
export default function CrearAnuncioPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        // Validar que no sea un anuncio enviado
        if (data.estado === "enviado") {
          setErrores(["No se puede editar un anuncio que ya fue enviado."]);
          setEstadoOriginal("enviado");
          return;
        }
  ...
  // Manejar cambio de título con sanitización
  ...
  // Manejar cambio de mensaje con sanitización
  ...
  function validar(esBorrador: boolean = false): {
    ...
    // Para borrador, solo necesitamos el título válido
    if (esBorrador) {
  ...
  async function handleGuardar(targetEstado: Estado) {
    ...
    // Validar según el estado objetivo
    ...
    // Si hay advertencias y no es borrador, pedir confirmación
    if (resultado.advertencias.length > 0 && !esBorrador) {
      // Las advertencias se muestran pero no bloquean
    }
    ...
    try {
      ...
      let result;
      if (anuncioId) {
        // Verificar que el anuncio aún existe y no fue enviado
  ...
  function handleVistaPrevia() {
    ...
    router.push(
      `/admin/anuncios/preview?titulo=${encodeURIComponent(
        titulo.trim()
      )}&mensaje=${encodeURIComponent(mensaje.trim())}`
    );
  }

  // Si el anuncio no se encontró
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-red-600 mt-2">
                El anuncio que intentas editar no existe o fue eliminado.
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  // Si es un anuncio enviado (no editable)
  if (estadoOriginal === "enviado") {
    return (
      <div className="p-4 md:p-6 space-y-6">
        <Button
          variant="outline"
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-yellow-600 mt-2">
                Este anuncio ya fue enviado y no se puede modificar.
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <Button
                ...
                disabled={isSaving || !titulo.trim() || !mensaje.trim()}
              >
