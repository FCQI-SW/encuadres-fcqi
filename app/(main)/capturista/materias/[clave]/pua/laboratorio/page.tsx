...
export default function PuaMateriaLaboratorio() {
  ...
  const [practicasPorUnidad, setPracticasPorUnidad] = useState<
    Record<number, PracticaLaboratorioType[]>
  >({});
  ...
  const { loading, guardarPracticas, cargarPracticas } = useLaboratorioForm(
    programa?.id || ""
  );
    ...
    const fetchPrograma = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          if (!programaData?.hl || Number(programaData.hl) <= 0) {
  ...
  const validarPracticas = (): Record<string, ErroresCampos> => {
    ...
    const todasLasPracticas: PracticaLaboratorioType[] = [];
  ...
  const handleGuardar = async () => {
    ...
    if (totalPracticas === 0) {
      toast.error(
        "Debes agregar al menos una práctica de laboratorio antes de continuar."
      );
      return;
    }
    ...
    const shouldSave = await confirm({
      ...
      message: "¿Deseas guardar todas las prácticas de laboratorio?",
      confirmText: "Guardar",
      cancelText: "Cancelar",
    });
    ...
    if (success) {
      await confirm({
        ...
        message:
          "Las prácticas de laboratorio se han guardado correctamente. El PUA está completado.",
      ...
      toast.error(
        "Error al guardar las prácticas de laboratorio. Intenta de nuevo."
      );
    }
  };
