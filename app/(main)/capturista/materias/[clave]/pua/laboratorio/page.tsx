...
export default function PuaMateriaLaboratorio() {
  ...
  const [practicasPorUnidad, setPracticasPorUnidad] = useState<Record<number, PracticaLaboratorioType[]>>({});
    ...
    const fetchPrograma = async () => {
      ...
      try {
        const { data: programaData, error } = await supabase
          .from("programas")
          .select("id, materia_id, unidades, hl")
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const agrupadas: Record<number, PracticaLaboratorioType[]> = {};
  ...
  const handleGuardar = async () => {
    ...
    if (totalPracticas === 0) { toast.error("Debes agregar al menos una práctica de laboratorio antes de continuar."); return; }

    const shouldSave = await confirm({ title: "Guardar prácticas", message: "¿Deseas guardar todas las prácticas de laboratorio?", confirmText: "Guardar", cancelText: "Cancelar" });
    if (!shouldSave || !programa) return;

    const success = await guardarPracticas(Object.values(practicasPorUnidad).flat());

    if (success) {
      await confirm({ title: "¡Guardado exitoso!", message: "Las prácticas de laboratorio se han guardado correctamente. El PUA está completado.", confirmText: "Finalizar", cancelText: "" });
      router.push(`/capturista/materias`);
    } else {
      toast.error("Error al guardar las prácticas de laboratorio. Intenta de nuevo.");
    }
  };
    ...
    <div className="px-4 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="text-center py-4">
          <h1 className="text-2xl font-bold">VI. ESTRUCTURA DE LAS PRÁCTICAS DE LABORATORIO</h1>
              ...
              {practicas.length === 0 ? (
                ...
                <div className="space-y-4">
                  {practicas.map((practica) => {
                    ...
                    return (
                      <PracticaLaboratorio key={key} unidad={unidad} numero={practica.numero} value={practica}
